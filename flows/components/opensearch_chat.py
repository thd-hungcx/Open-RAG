...
def normalize_index_name(index_name: str) -> str:
    """Normalize an index name into a safe OpenSearch identifier."""
    value = (index_name or "").strip().lower()
...
class OpenSearchVectorStoreComponentMultimodalMultiEmbedding(LCVectorStoreComponent):
    ...
    inputs = [
        ...
        *LCVectorStoreComponent.inputs,  # includes search_query, add_documents, etc.
        ...
        BoolInput(
            name="enable_chunk_limit",
            display_name="Enable Chunk Limit",
            value=False,
            advanced=False,
            info="When enabled, cap the number of returned chunks by Max Chunks.",
        ),
        IntInput(
            name="max_chunks",
            ...
            value=5,
            advanced=False,
            info="Maximum number of chunks to return when Enable Chunk Limit is on.",
        ),
        MultilineInput(
    ...
    def raw_search(self, query: str | dict | None = None) -> Data:
        ...
        resp = client.search(
            index=self._resolve_index_name(),
            body=query_body,
    ...
    def _add_documents_to_vector_store(self, client: OpenSearch) -> None:
        ...
        resolved_index = self._resolve_index_name()
        ...
        try:
            if not client.indices.exists(index=resolved_index):
                self.log(f"Creating index '{resolved_index}' with base mapping")
                client.indices.create(index=resolved_index, body=mapping)
            ...
            if creation_error.error == "resource_already_exists_exception":
                ...
                if "invalid engine" in error_msg or "illegal_argument" in error_msg:
                    ...
                    if "index.knn" in error_msg:
                        ...
                        raise ValueError(msg) from creation_error
                logger.warning(
                    f"Failed to create index '{resolved_index}': {creation_error}"
                )
                raise
        ...
        self.log(
            f"Indexing {len(texts)} documents into '{resolved_index}' with model '{embedding_model}'..."
        )
    ...
    def _build_rbac_filter_clause(self, filter_obj: dict | None) -> dict | None:
        ...
        department_context = self._extract_context_text(
            getattr(self, "department_context", "")
        )

        user_department = (
            str(filter_obj.get("department") or "").strip() or department_context
        )
        if not user_department:
            return None

        role = (
            str(filter_obj.get("role") or "").strip() or role_context or "employee"
        ).lower()
    ...
    def _resolve_index_name(self) -> str:
        """Resolve target index, prioritizing department_context for chat search."""
        # Priority 1: department in OPENRAG-QUERY-FILTER (set by backend headers)
        filter_obj = self._parse_filter_expression_value()
        if isinstance(filter_obj, dict):
            department_from_filter = str(filter_obj.get("department") or "").strip()
            if department_from_filter:
                resolved = normalize_index_name(department_from_filter)
                if resolved:
                    logger.info(
                        f"[OpenSearchMultimodel-Chat] Resolved index from filter department: '{resolved}'"
                    )
                    return resolved
        ...
        if department_context:
            resolved = normalize_index_name(department_context)
            if resolved:
                logger.info(
                    f"[OpenSearchMultimodel-Chat] Resolved index from department_context: '{resolved}'"
                )
                return resolved

        fallback = normalize_index_name(str(getattr(self, "index_name", "") or ""))
        resolved = fallback or "documents"
        logger.info(
            f"[OpenSearchMultimodel-Chat] Resolved index from fallback: '{resolved}'"
        )
        return resolved
    ...
    def _parse_filter_expression_value(self) -> dict:
        ...
        if not isinstance(raw_filter, str):
            # Handle Message/Data-like objects
            text_attr = getattr(raw_filter, "text", None)
            if isinstance(text_attr, str):
                raw_filter = text_attr
            else:
                data_attr = getattr(raw_filter, "data", None)
                if isinstance(data_attr, dict):
                    nested_text = data_attr.get("text")
                    if isinstance(nested_text, str):
                        raw_filter = nested_text
                    else:
                        return {}
                else:
                    return {}
    ...
    def _detect_available_models(
        ...
        try:
            ...
            if not models:
                ...
                logger.warning(
                    f"No embedding_model values found in index '{self._resolve_index_name()}'. "
    ...
    def _get_index_properties(self, client: OpenSearch) -> dict[str, Any] | None:
        ...
        try:
            mapping = client.indices.get_mapping(index=self._resolve_index_name())
        except OpenSearchException as e:
            logger.warning(
                f"Failed to fetch mapping for index '{self._resolve_index_name()}': {e}. Proceeding without mapping metadata."
            )
            return None
    ...
    def _resolve_terms_agg_field(
        self, field_name: str, index_properties: dict[str, Any] | None
    ) -> str | None:
        ...
        for agg_name, (field_name, size) in agg_specs.items():
            ...
            if resolved_field:
                aggregations[agg_name] = {
                    "terms": {"field": resolved_field, "size": size}
                }
            else:
                self.log(
                    f"[AGG SKIP] Skipping aggregation '{agg_name}' - field '{field_name}' is not keyword-compatible"
                )
    ...
    def search(self, query: str | None = None) -> list[dict[str, Any]]:
        ...
        raw_limit = (filter_obj or {}).get("limit", self.number_of_results)
        try:
            limit = int(raw_limit)
        except (TypeError, ValueError):
            limit = int(self.number_of_results)
        if limit <= 0:
            limit = int(self.number_of_results)
        score_threshold = (filter_obj or {}).get("score_threshold", 0)

        # Optional chunk cap override
        enable_chunk_limit = bool(getattr(self, "enable_chunk_limit", False))
        if enable_chunk_limit:
            try:
            ...
            if max_chunks > 0:
                ...
                self.log(f"[CHUNK LIMIT] enabled -> returning at most {limit} chunks")
        ...
        try:
            resp = client.search(
                index=resolved_index, body=body, params={"terminate_after": 0}
            )
        except RequestError as e:
            error_message = str(e)
