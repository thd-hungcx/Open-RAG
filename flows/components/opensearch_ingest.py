...
class OpenSearchVectorStoreComponentMultimodalMultiEmbedding(LCVectorStoreComponent):
    ...
    def _resolve_index_name(self) -> str:
        """Resolve index from component input with normalization and fallback."""
        raw: Any = getattr(self, "index_name", "")
        ...
        if isinstance(raw, dict):
            raw = raw.get("text") or raw.get("value") or raw.get("input_value") or ""

        # Serialized JSON-like message
        ...
        resolved = normalize_index_name(str(raw))
        if not resolved:
            fallback = normalize_index_name(
                os.getenv("OPENSEARCH_INDEX_NAME", "documents")
            )
            resolved = fallback or "documents"

        logger.info(f"[OpenSearchMultimodel] Resolved index name: '{resolved}'")
        return resolved
